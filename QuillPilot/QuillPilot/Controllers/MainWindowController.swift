...
extension MainWindowController {
    ...
    private func importFile(url: URL) throws {
        ...
        case "docx":
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                // Always prefer the custom DOCX extractor. It preserves paragraph indents
                // and style identity that the system OfficeOpenXML importer can drop
                // (which causes Body Text to reopen as "Body Text – No Indent").
                // Fall back to the system importer only if the custom parser fails.
                do {
                    ...
                    // Custom parser failed — fall back to the system importer below.
                }
    ...
    private func applyImportedContent(_ attributed: NSAttributedString, url: URL) {
        ...
        // Check whether the imported content already carries QuillStyleName attributes
        // (e.g. from our custom DOCX parser). System importers (ODT, RTF,
        // officeOpenXML fallback) don't set them, so we need style inference & repair.
        let hasQuillStyles: Bool = {
            let key = NSAttributedString.Key("QuillStyleName")
            var found = false
            attributed.enumerateAttribute(key, in: NSRange(location: 0, length: min(attributed.length, 5000)), options: []) { value, _, stop in
        ...
        // When content already has QuillStyleName tags (custom parser): non-destructive open,
        // but always repair Body Text indent as a safety net.
        // When it doesn't (system importer): enable retagging & repair so styles render correctly.
        mainContentViewController.editorViewController.setAttributedContentDirect(
            attributed,
            retagStylesForOutline: !hasQuillStyles,
            materializeCatalogStyles: !hasQuillStyles,
            repairBodyIndent: true,
            repairTOCIndexFormatting: !hasQuillStyles
        )
...
private enum DocxBuilder {
    ...
    private static func paragraphPropertiesXml(from style: NSParagraphStyle?, styleName: String? = nil) -> String {
        ...
        if let style = style {
            ...
            // Export tab stops only for TOC/Index styles (right-aligned page columns).
            // Body Text and other regular styles may carry default NSParagraphStyle tab stops
            // that would pollute the DOCX and cause byClipping on reimport.
            let isTocOrIndexStyle: Bool = {
                guard let name = styleName?.lowercased() else { return false }
                return name.contains("toc") || name.contains("index")
            }()
            if !style.tabStops.isEmpty && isTocOrIndexStyle {
    ...
    private final class DocumentXMLAttributedCollector: NSObject, XMLParserDelegate {
        ...
        func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String: String] = [:]) {
            ...
            case "w:pstyle", "pstyle":
                if let val = attributeDict["w:val"] ?? attributeDict["val"] {
                    ...
                    case "PoetryAuthor": mappedName = "Poetry — Author"
                    case "PoetryPoetName": mappedName = "Poetry — Poet Name"
                    case "PoetryStanzaBreak": mappedName = "Poetry — Stanza Break"
                    case "SectionSequenceTitle": mappedName = "Section / Sequence Title"
                    case "PartNumber": mappedName = "Part Number"
                    case "SectionBreak": mappedName = "Section Break"
                    case "ProsePoem": mappedName = "Prose Poem"
                    case "VerseParagraph": mappedName = "Verse Paragraph"
                    case "MarginalNote": mappedName = "Marginal Note"
                    case "DraftMarginNote": mappedName = "Draft / Margin Note"
                    case "ArgumentTitle": mappedName = "Argument Title"
                    case "RevisionVariant": mappedName = "Revision Variant"
                    // Standard structure styles
                    case "ImageCaption": mappedName = "Image Caption"
                    case "FictionCharacter": mappedName = "Fiction — Character"
                    case "TableTitle": mappedName = "Table Title"
                    case "GlossaryTitle": mappedName = "Glossary Title"
                    case "GlossaryEntry": mappedName = "Glossary Entry"
                    case "AppendixTitle": mappedName = "Appendix Title"
            ...
            if paragraphBuffer.length > 0 {
                ...
                if let styleDefinition = StyleCatalog.shared.style(named: styleName) {
                    ...
                    if let merged = updatedParagraphStyle.mutableCopy() as? NSMutableParagraphStyle {
                        // Preserve the first-line indent parsed from the DOCX XML.
                        // The catalog may differ from what was actually saved (e.g. "Body Text" vs
                        // "Body Text – No Indent"), so trust the document's own indentation.
                        merged.headIndent = paragraph.headIndent
                        merged.firstLineHeadIndent = paragraph.firstLineHeadIndent
                        merged.tailIndent = paragraph.tailIndent

                        // Preserve imported tab stops for TOC/Index leader-dot formatting.
                        // Only carry tab stops for TOC/Index styles — regular paragraphs
                        // may accumulate default tab stops that cause byClipping on reopen.
                        let isTocOrIndex = styleName.lowercased().contains("toc") || styleName.lowercased().contains("index")
                        if !paragraph.tabStops.isEmpty && isTocOrIndex {
                            merged.tabStops = paragraph.tabStops
                            merged.defaultTabInterval = paragraph.defaultTabInterval
                        }
                        // Preserve non-wrapping behavior for TOC/Index lines only.
                        if paragraph.lineBreakMode == .byClipping && isTocOrIndex {
        ...
        private struct ParagraphStyleProps {
            ...
            func makeParagraphStyle() -> NSParagraphStyle {
                ...
                // Only use byClipping for TOC/Index entries (which need right-aligned
                // tab stops for page numbers).  Regular paragraphs that happen to carry
                // tab stops (e.g. from a DOCX round-trip) must still word-wrap.
                if (styleName?.hasPrefix("TOC") == true) || (styleName?.hasPrefix("Index") == true) {
